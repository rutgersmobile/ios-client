...
open class RUFavorite: NSObject
{
    ...
    // If two things are equal then there hashes must be equal :
    // but two hashes being equal does not mean that the objects are equal
...
public let MenuItemManagerDidChangeActiveMenuItemsKey =
    "MenuItemManagerDidChangeActiveMenuItemsKey"
...
open class RUMenuItemManager: NSObject {
    ...
    public func addFavorite(favorite: RUFavorite) {
        ...
        let optionalIndex = rawMenuItems.index {
            $0.object.isEqual(favoriteDict)
        }
        if optionalIndex == nil {
            rawMenuItems.insert(VisibleObject(
                visible: true,
                object: favoriteDict
            ), at: 0)
        }
    ...
    fileprivate func notifyMenuItemsDidChange() {
        NotificationCenter.default.post(
            name: Notification.Name(
                rawValue: MenuItemManagerDidChangeActiveMenuItemsKey
            ),
            object: self
        )
    }

    private func convertOld(
        bookmark: Any
    ) -> Dictionary<String, AnyObject>? {
        if let channel = bookmark as? String {
        ...
        if let dict = bookmark as? Dictionary<String, AnyObject> {
    ...
    private var rawMenuItems: [VisibleObject] {
        set {
            UserDefaults.standard.set(
                newValue.map { $0.asDict() },
                forKey: MenuItemManagerActiveMenuItemsKey
            )
            notifyMenuItemsDidChange()
        }
        get {
            let contentChannels = RUChannelManager
                .sharedInstance().contentChannels as [AnyObject]
                ...
                .array(forKey: MenuItemManagerActiveMenuItemsKey)
            {
                ...
                let dictDefaults = oldDefaults.filterMap {
                    convertOld(bookmark: $0)
                }
                let channelHandles = contentChannels.flatMap {
                    $0.channelHandle
                }
                ...
                return contentChannels.map {
                    VisibleObject(
                        visible: true,
                        object: $0.channelHandle as AnyObject
                    )
                }
            }
        }
    ...
    private func fullItem(object: AnyObject) -> AnyObject? {
        ...
        case let string as String:
            return RUChannelManager
                .sharedInstance()
                .channel(withHandle: string) as AnyObject?
        default: return nil
        }
    }

    private func serializeNew(
        objects: [AnyObject],
        visible: Bool
    ) -> [VisibleObject] {
    ...
    public func updateItems(visible: [AnyObject], hidden: [AnyObject]) {
        rawMenuItems
            = serializeNew(objects: visible, visible: true)
            + serializeNew(objects: hidden, visible: false)
    }

    public var menuItems: [AnyObject] {
        set {
            rawMenuItems
                = serializeNew(objects: newValue, visible: true)
                + rawHidden()
        }
        get {
    ...
    public var hiddenItems: [AnyObject] {
        set {
            rawMenuItems
                = rawVisible()
                + serializeNew(objects: newValue, visible: false)
        }
        get {
